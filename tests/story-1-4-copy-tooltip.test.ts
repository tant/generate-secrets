...
test.describe('Story 1.4: Copy and Tooltip Features', () => {
  ...
  test('should display ? icons next to each secret label', async ({ page }) => {
    ...
    for (const secretLabel of requiredSecrets) {
      ...
      const tooltipButton = labelElement.locator('..').getByRole('button', { name: `Show explanation for ${secretLabel}` });
      await expect(tooltipButton).toBeVisible();
    }
  });

  test('should display tooltip with correct explanation when ? icon is clicked', async ({ page }) => {
    // Generate secrets first
    await page.getByRole('button', { name: 'Generate' }).click();
    ...
    const usernameTooltipButton = page.getByText('username:', { exact: true }).locator('..').getByRole('button', { name: 'Show explanation for username' });
    ...
    await expect(tooltip).toContainText('Random, readable string, 8-12 characters, no special characters.');
  });
  ...
  test('should copy all secrets to clipboard in .env format', async ({ page }) => {
    // Generate secrets first
    await page.getByRole('button', { name: 'Generate' }).click();
  ...
  test('should show error message when copying without generating secrets', async ({ page }) => {
    // Click the Copy button without generating secrets first
    await page.getByRole('button', { name: 'Copy' }).click();
    
    // Check for error message
    await expect(page.getByText('No secrets to copy. Please generate secrets first.')).toBeVisible();
  });
});